...
{
    public class clsPurchase : DBConnection
    {
        Common Comm = new Common();

        public string PurchaseMasterCRUD(clsJSonPurchase clsPur, SqlConnection sqlConn, SqlTransaction trans, string strJson = "", int iAction = 0)
        {
            ...
            //{
                try
                {
                    using (SqlCommand sqlCmd = new SqlCommand("UspPurchaseInsert", sqlConn, trans))
                    {
        ...
        //public string PurchaseDetailCRUD(clsJSonPurchase clsPur, string sBatchCode = "", int iAction = 0)
        //{
        //    DataTable dtResult = new DataTable();
        //    string sResult = "";
        //    string sResultID = "";
        ...
        public string PurchaseDetailCRUD(clsJSonPurchase clsPur, SqlConnection sqlConn, SqlTransaction trans, string sBatchCode = "", int iAction = 0)
        {
            DataTable dtResult = new DataTable();
            ...
            //{
                try
                {
                    if (iAction == 0)
                    {
                        ...
                        {
                            using (SqlCommand sqlCmd = new SqlCommand("UspPurchaseItemInsert", sqlConn, trans))
                            {
        ...
        public DataTable GetPurchaseMaster(UspGetPurchaseInfo Info, bool blnIsPrevNext = false)
        {
            ...
            try
            {
                ...
                {
                    using (SqlDataAdapter sqlda = new SqlDataAdapter("UspGetPurchaseMaster", sqlcon))
                    {
        ...
        public DataTable GetPurchaseDetailItem(UspGetPurchaseInfo Info)
        {
            DataTable dtbl = new DataTable();
            try
            {
                using (var sqlcon = GetDBConnection())
                {
                    using (SqlDataAdapter sqlda = new SqlDataAdapter("UspGetPurchaseDetailItem", sqlcon))
                    {
                        sqlda.SelectCommand.CommandType = CommandType.StoredProcedure;
                        sqlda.SelectCommand.Parameters.Add("@InvID", SqlDbType.Decimal).Value = Info.InvId;
                        sqlda.Fill(dtbl);
