...
{
    public class clsPriceList : DBConnection
    {
        Common Comm = new Common();

        public string PriceListMasterCRUD(clsJSonPriceList clsStockJournal, SqlConnection sqlConn, SqlTransaction trans, string strJson = "", int iAction = 0)
        {
            ...
            try
            {
                using (SqlCommand sqlCmd = new SqlCommand("UspPriceListMasterInsert", sqlConn, trans))
                {
        ...
        public string PriceListDetailCRUD(clsJSonPriceList clsPriceList, SqlConnection sqlConn, SqlTransaction trans, string sBatchCode = "", int iAction = 0)
        {
            DataTable dtResult = new DataTable();
            string sBatchCodeNew = "";
            DataTable dbtl = clsPriceList.clsJsonSJDetailsInfoList_.ToDataTable();
            try
            {
                if (iAction == 0)
                {
                    ...
                    {
                        using (SqlCommand sqlCmd = new SqlCommand("UspPriceListDetailInsert", sqlConn, trans))
                        {
                            sqlCmd.CommandType = CommandType.StoredProcedure;
                            SqlDataAdapter sqlDa = new SqlDataAdapter(sqlCmd);
                            ...
                            SpParam = sqlCmd.Parameters.Add("@InvID", SqlDbType.Decimal);
                            SpParam.Value = clsPriceList.clsJsonPLInfo_.InvID; //Convert.ToDecimal(dbtl.Rows[i]["InvID"].ToString());
                ...
                else if (iAction == 2)
                {
                    using (SqlCommand sqlCmd = new SqlCommand("UspPriceListDetailInsert", sqlConn, trans))
                    {
                        ...
                        SpParam = sqlCmd.Parameters.Add("@InvID", SqlDbType.Decimal);
                        SpParam.Value = clsPriceList.clsJsonPLInfo_.InvID; //Convert.ToDecimal(dbtl.Rows[0]["InvID"].ToString());
                        SpParam = sqlCmd.Parameters.Add("@ItemID", SqlDbType.Int);
        ...
        public DataTable GetPriceListMaster(UspGetPriceListInfo Info, bool blnIsPrevNext = false)
        {
            ...
            try
            {
                ...
                {
                    using (SqlDataAdapter sqlda = new SqlDataAdapter("UspGetPriceListMaster", sqlcon))
                    {
        ...
        public DataTable GetPriceLisDetailItem(UspGetPriceListInfo Info)
        {
            DataTable dtbl = new DataTable();
            try
            {
                using (var sqlcon = GetDBConnection())
                {
                    using (SqlDataAdapter sqlda = new SqlDataAdapter("UspGetPriceListDetail", sqlcon))
                    {
                        sqlda.SelectCommand.CommandType = CommandType.StoredProcedure;
                        sqlda.SelectCommand.Parameters.Add("@InvID", SqlDbType.Decimal).Value = Info.InvId;
                        sqlda.Fill(dtbl);
