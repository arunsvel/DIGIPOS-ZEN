...
{
    public partial class frmCompactSearch : Form
    {
        frmMDI mMDIParent = null;
        public frmCompactSearch(dlgt_CompSearch btnSearches, string strQuery = "", string strSearchFieldNames = "", int XPos = 0, int YPos = 0, int iSelectIDPos = 0, int iFieldCount = 0, string sEnterData = "", int iHideColIndexFrom = 0, int iType = 0, string strOrderBy = "", int iCompactSearchWidth = 0, int iShowTopCnt = 0, string sTitle = "", int iSearchColindex = -1, string sColsWidth = "", bool bShowWholeDataWhenNoSearch = false, string sFormName = "", int FirstShowRecordCnt = 0, bool bIsNumericOnly = false, object MDIParent = null)
        {
            InitializeComponent();
        ...
        int xAxisPos = 0, yAxisPos = 0, iSelIDPos = 0, iFieldCnt = 0, iHideColumnFrom, iCompSearchWidth, iSearchColumnIndex, iShowCnt;
        ...
        #region "EVENTS ------------------------------------------------ >>"
        private void frmCompactSearch_MouseDown(object sender, MouseEventArgs e)
        {
            ReleaseCapture();
        ...
        private void FrmCompactSearch_Load(object sender, EventArgs e)
        {
        ...
        private void FrmCompactSearch_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape)
            {
                Boolean result = btnSearchClick("0|" + txtSearch.Text + "");
        ...
        private void frmCompactSearch_Shown(object sender, EventArgs e)
        {
            txtSearch.Focus();
            if (txtSearch.Text.Length > 0)
                txtSearch.Select(txtSearch.Text.Length, 1);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            else if (strFormName.Trim() == "frmCustomer")
            {
                frmLedger frmLed = new frmLedger(0, true, 0, "CUSTOMER", txtSearch, true);
                frmLed.ShowDialog();
                //frmLed.BringToFront();
        ...
        private void btnEdit_Click(object sender, EventArgs e)//Added by Anjitha
        {
            ...
            {
                frmLedger frmLed = new frmLedger(sVal, true, 0, "CUSTOMER", txtSearch, true);
                frmLed.ShowDialog();
            }
            //Added by Anjitha 02/03/2022
            else if (strFormName.Trim() == "frmAreaMaster")
        ...
        private void WhenDgvSearchEnter()
        {
            ...
            if (dgvSearch.SelectedRows.Count > 0)
            {
                ...
                {
                    //WhenClickGo(sVal + "|" + sSearchText);
                    if (strFormName == "BatchCode")
                    {
                        WhenClickGo(dgvSearch.CurrentRow.Cells[0].Value.ToString() + "|" + sVal );
                    }
                    else
                    {
                        WhenClickGo(sVal + "|" + sSearchText);
