...
namespace InventorSync
{
    public partial class frmUser : Form, IMessageFilter
    {
        ...
        private void sfcboLedger_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.Shift == true && e.KeyCode == Keys.Enter)
        ...
        private void sfcboLedger_Enter(object sender, EventArgs e)
        {
            Comm.ControlEnterLeave(sfcboLedger, true);
        }
        private void sfcboLedger_Leave(object sender, EventArgs e)
        {
            Comm.ControlEnterLeave(sfcboLedger, false, false);
        }
        ...
        private void sfcboLedger_SelectedIndexChanged(object sender, EventArgs e)
        {
            sfcboLedger.Tag = sfcboLedger.SelectedValue;
        }
        ...
        private void btnAddLedg_Click(object sender, EventArgs e)
        {
            try
            {
                frmLedger frmLed = new frmLedger(0, false, 20, "", sfcboLedger);
                frmLed.Show();
            }
            catch (Exception ex)
            {
        ...
        private void btnEditLedg_Click(object sender, EventArgs e)
        {
            try
            {
                frmLedger frmLed = new frmLedger(Convert.ToInt32(sfcboLedger.Tag), true, 20, "", sfcboLedger);
                frmLed.Show();
            }
            catch (Exception ex)
            {
        ...
        private void LoadStaffLedger()
        {
            ...
            sfcboLedger.DataSource = dtGroup;
            sfcboLedger.DisplayMember = "LName";
            sfcboLedger.ValueMember = "LID";
        }
        ...
        private void LoadData(int iSelectedID = 0)
        {
            try
            {
                ...
                if (dtLoad.Rows.Count > 0)
                {
                    ...
                    sfcboLedger.Tag= dtLoad.Rows[0]["UserLedgerID"].ToString();
                    if (dtLoad.Rows[0]["UserLedgerID"].ToString() != "")
                        sfcboLedger.SelectedValue = Convert.ToDecimal(dtLoad.Rows[0]["UserLedgerID"].ToString());
        ...
        private void SaveData()
        {
            if (IsValidate() == true)
            {
                ...
                if (sfcboLedger.Tag == "")
                    sfcboLedger.Tag = 0;
                UserInfo.UserLedgerID = Convert.ToDecimal(sfcboLedger.Tag);
        ...
        private void DeleteData()
        {
            ...
            UserInfo.SalesReturnVchtypeIDs = "";
            UserInfo.AccountsVchtypeIDs = "";
            UserInfo.UserLedgerID = Convert.ToDecimal(sfcboLedger.SelectedValue);
            if (toggleCountermgmt.ToggleState == Syncfusion.Windows.Forms.Tools.ToggleButtonState.Active)
                iActiveCountermgmt = 1;
            else
                iActiveCountermgmt = 0;
