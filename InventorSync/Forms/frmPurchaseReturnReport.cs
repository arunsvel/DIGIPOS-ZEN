...
namespace InventorSync
{
    public partial class frmPurchaseReturnReport : Form
    {
        ...
        {
            rdoPurchaseDaybook.Checked = true;
        ...
        private void txtVoucherTypeList_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (this.ActiveControl.Name != "txtVoucherTypeList")
                    return;
                string sQuery = "Select VchTypeID,VchType from tblVchType where ParentID=4 AND TenantID = '" + Global.gblTenantID + "'";
        ...
        private void chkVoucher_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string Sql = "Select VchTypeID,VchType from tblVchType where ParentID = 4 AND TenantID = '" + Global.gblTenantID + "'";
                    SqlConnection conn = new SqlConnection(constr);
                    conn.Open();
                    SqlCommand cmd = new SqlCommand(Sql, conn);
        ...
        private void chkBillType_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (chkBillType.Checked == true)
                {
                    ...
                    string Sql = "Select Distinct 0 as BID,GSTType as BillTypeName from tblPurchase Order By GSTType ";
                    SqlConnection conn = new SqlConnection(constr);
                    conn.Open();
                    SqlCommand cmd = new SqlCommand(Sql, conn);
                    DataTable dt = new DataTable();
        ...
        private void txtBillType_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.ActiveControl.Name != "txtBillType")
                    return;
                string sQuery = "Select Distinct GSTType as BID,GSTType as BillTypeName from tblPurchase Order By GSTType";
        ...
        private void chkMop_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string Sql = "Select distinct 0 as MID, mop as MopName from tblPurchase Order By MOP";
                    SqlConnection conn = new SqlConnection(constr);
                    conn.Open();
                    SqlCommand cmd = new SqlCommand(Sql, conn);
                    DataTable dt = new DataTable();
        ...
        private void txtSupplier_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                new frmCompactSearch(GetFromSupplierSearch, sQuery, "AnyWhere|LAliasName|LName|MobileNo|Address", txtSupplier.Location.X + 480, txtSupplier.Location.Y - 10, 4, 0, txtSupplier.Text, 4, 0, "ORDER BY L.LAliasName ASC", 0, 0, " Search ...", 0, "100,200,100,200,0", true, "").ShowDialog();
        ...
        private void rdoPurchaseDaybook_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                //string sQuery = "SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'tblPurchase'";
        ...
        private void rdoDaybookDetails_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                chkselectAll.Checked = false;
                chkfield.Visible = true;
                string sQuery = "SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'tblPurchase' and ORDINAL_POSITION in(2,5,6,7,10,28,11,12,13,14,16,18,19,20,21,24,31,33,42,43,44,45,78,79,80,83)";
                DataTable dtCCntr = Comm.fnGetData(sQuery).Tables[0];
            ...
            try
            {
                ...
                if (chkfield.CheckedItems.Count != 0)
                {
                    ...
                    for (int x = 0; x < chkfield.CheckedItems.Count; x++)
                    {
                        ...
                        {
                            s = s + "tblPurchaseItem." + chkfield.GetItemText(chkfield.CheckedItems[x]) + ",";
                        }
                        ...
                        else if (chkfield.GetItemText(chkfield.CheckedItems[x]) == "Taxable")
                        {
                            s = s + "cast(Taxable as numeric(36,2)) as [Taxable],";
                        }
                ...
                if (rdoPurchaseDaybook.Checked == true)
                {
                    DropPurchaseView();
                    string Fname = "Purchase Return Daybook";
                    ...
                    if (rdoDefault.Checked == true)
                    {
                        ...
                        if (txtSupplier.Text != "")
                        {
                            sqlSupplier = " tblPurchase.LedgerId = " + Conversion.Val(lblLID.Text) + "  AND ";
                        }

                        string Sql1 = "create view vwpurchase as SELECT " + d +
                                        " " +
                                        " FROM   tblpurchase INNER JOIN tblcostcentre    ON tblcostcentre.ccid = tblpurchase.ccid" +
                                        " WHERE " + sqlSupplier + " invdate BETWEEN '" + FD.ToString("dd-MMM-yyyy") + "' and '" + TD.ToString("dd-MMM-yyyy") + "'  AND tblPurchase.VchTypeID IN (" + lblVoucherIds.Text + ") " +
                                        " AND tblPurchase.TaxModeID IN (" + lblTaxIds.Text + ")   AND tblPurchase.billtype IN (" + txtBillType.Text + ")   AND mop  IN (" + txtMop.Text + ")" +
                                        " AND salesmanid         IN (" + lblstaffIds.Text + ") " +
                                        " AND tblpurchase.ccid IN (" + lblCostCenterIds.Text + ") " +
                                        " AND tblpurchase.userid IN (" + lblUserIds.Text + ") " +
                ...
                if (rdoDaybookDetails.Checked == true)
                {
                    ...
                    string Fname = "Purchase Return Daybook Details";
                    ...
                    if (rdoDefault.Checked == true)
                    {
                        ...
                        string Sql1 = "create view vwpurchase as SELECT " + d +
                                         " " +
                                         " FROM   tblpurchase INNER JOIN tblPurchaseitem ON tblpurchase.InvId=tblPurchaseitem.InvId INNER JOIN tblItemMaster on tblItemMaster.ItemID=tblPurchaseItem.ItemId INNER JOIN tblcostcentre ON tblcostcentre.ccid = tblpurchase.ccid" +
                                         " WHERE " + sqlSupplier + " invdate BETWEEN '" + FD.ToString("dd-MMM-yyyy") + "' and '" + TD.ToString("dd-MMM-yyyy") + "'  AND tblPurchase.VchTypeID IN (" + lblVoucherIds.Text + ") " +
                                         " AND tblPurchase.TaxModeID IN (" + lblTaxIds.Text + ")   AND tblPurchase.billtype IN (" + txtBillType.Text + ")   AND mop  IN (" + txtMop.Text + ")" +
                                         " AND salesmanid         IN (" + lblstaffIds.Text + ") " +
                                         " AND tblpurchase.ccid IN (" + lblCostCenterIds.Text + ") " +
                ...
                if (rdbHsncode.Checked == true)
                {
                    ...
                    string Fname = "Purchase Return Hsncode Wise";
                ...
                if (rdbItems.Checked == true)
                {
                    ...
                    string Fname = "Purchase Return Item Wise";
                    ...
                    if (rdoDefault.Checked == true)
                    {
                        ...
                        string Sql1 = "create view vwpurchase as SELECT " + d +
                                         " " +
                                         " FROM   tblpurchase INNER JOIN tblPurchaseitem ON tblpurchase.InvId=tblPurchaseitem.InvId INNER JOIN tblItemMaster on tblItemMaster.ItemID=tblPurchaseItem.ItemId INNER JOIN tblcostcentre ON tblcostcentre.ccid = tblpurchase.ccid INNER JOIN tblCategories on tblCategories.CategoryID = tblItemMaster.CategoryID" +
                                         " WHERE " + sqlSupplier + " invdate BETWEEN '" + FD.ToString("dd-MMM-yyyy") + "' and '" + TD.ToString("dd-MMM-yyyy") + "'  AND tblPurchase.VchTypeID IN (" + lblVoucherIds.Text + ") " +
                                         " AND " + sqlitem + " tblPurchase.TaxModeID IN (" + lblTaxIds.Text + ")   AND tblPurchase.billtype IN (" + txtBillType.Text + ")   AND mop  IN (" + txtMop.Text + ")" +
                                         " AND salesmanid         IN (" + lblstaffIds.Text + ") " +
                                         " AND tblpurchase.ccid IN (" + lblCostCenterIds.Text + ") " +
                                         " AND tblpurchase.userid IN (" + lblUserIds.Text + ") " +
                                         " AND agentid            IN (" + lblAgentIds.Text + ") " +
                ...
                if (rdbTax.Checked == true)
                {
                    ...
                    string Fname = "Purchase Return Tax split";
                    ...
                    if (rdoDefault.Checked == true)
                    {
                        ...
                        string Sql1 = "create view vwpurchase as SELECT " + d +
                                         " " +
                                         " FROM   tblpurchase INNER JOIN tblPurchaseitem ON tblpurchase.InvId=tblPurchaseitem.InvId  INNER JOIN tblcostcentre ON tblcostcentre.ccid = tblpurchase.ccid" +
                                         " WHERE " + sqlSupplier + " invdate BETWEEN '" + FD.ToString("dd-MMM-yyyy") + "' and '" + TD.ToString("dd-MMM-yyyy") + "'  AND tblPurchase.VchTypeID IN (" + lblVoucherIds.Text + ") " +
                                         " AND tblPurchase.TaxModeID IN (" + lblTaxIds.Text + ")   AND tblPurchase.billtype IN (" + txtBillType.Text + ")   AND mop  IN (" + txtMop.Text + ")" +
                                         " AND salesmanid         IN (" + lblstaffIds.Text + ") " +
                                         " AND tblpurchase.ccid IN (" + lblCostCenterIds.Text + ") " +
        ...
        private void rdoPurchaseDaybook_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                chkfield.Visible = true;
        ...
        private void rdoDaybookDetails_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                //string Sql = "DROP VIEW vwpurchase";
                //string Sql1 = "create view vwpurchase as select top 1 Mop,VchType,Party,PartyCode,PartyAddress,PartyGSTIN,BillType,ItemCode,ItemName,tblPurchaseItem.Qty,tblPurchaseItem.PRate,TaxAmt,GrossAmt,ItemDiscountTotal,NonTaxable,Taxable,CGSTTotal,SGSTTotal,IGSTTotal,FloodCessTot,CashDiscount,OtherExpense,NetAmount,RoundOff,BillAmt from tblPurchase join tblPurchaseItem on tblPurchase.InvId = tblPurchaseItem.InvID join tblItemMaster on tblItemMaster.ItemID = tblPurchaseItem.ItemId";

                string Sql1 = "create view vwpurchase as select top 1 CCName as [Cost Center],Mop,VchType as [Voucher Type],Party,PartyCode as [Party Code] ,PartyAddress as [Party Address],PartyGSTIN as [Party GSTIN],BillType as [Bill Type],ItemCode as [Item Code],ItemName as [Item Name],tblPurchaseItem.Qty as Qty,tblPurchaseItem.PRate as PRate,TaxAmt as [Tax Amount],GrossAmt as [Gross Amount],ItemDiscountTotal as [Item Discount Total],NonTaxable as [Non Taxable],Taxable,CGSTTotal as [CGST Total],SGSTTotal as[SGST Total],IGSTTotal as [IGST Total],CashDiscount as[Cash Discount],OtherExpense as [Other Expense],NetAmount as [Net Amount],RoundOff as [Round Off],BillAmt as [Bill Amount] from tblPurchase join tblPurchaseItem on tblPurchase.InvId = tblPurchaseItem.InvID join tblItemMaster on tblItemMaster.ItemID = tblPurchaseItem.ItemId join tblCostCentre on tblCostCentre.CCID=tblPurchase.CCID";
                SqlConnection conn = new SqlConnection(constr);
                conn.Open();
                //SqlCommand cmd = new SqlCommand(Sql, conn);
                SqlCommand cmd1 = new SqlCommand(Sql1, conn);
        ...
        private void rdbItems_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //string Sql = "DROP VIEW vwpurchase";
                string Sql1 = "create view vwpurchase as select top 1 ItemCode as [Item Code],ItemName as [Item Name],Category,Manufacturer,ProductType as [Product Type],tblPurchaseItem.Qty,tblPurchaseItem.PRate,tblPurchaseItem.MRP,ItemDiscount as [Item Discount],HSNID as HsnCode,TaxPer,NonTaxable as [Non Taxable],Taxable,GrossAmount as [Gross Amount],TaxAmount as [Tax Amount],NetAmount as [Net Amount] from tblPurchase join tblPurchaseItem on tblPurchase.InvId = tblPurchaseItem.InvID join tblItemMaster on tblItemMaster.ItemID = tblPurchaseItem.ItemId join tblCategories on tblCategories.CategoryID=tblItemMaster.CategoryID";
                SqlConnection conn = new SqlConnection(constr);
                conn.Open();
                //SqlCommand cmd = new SqlCommand(Sql, conn);
                SqlCommand cmd1 = new SqlCommand(Sql1, conn);
        ...
        private void rdbHsncode_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DropPurchaseView();
                //string Sql = "DROP VIEW vwpurchase";
                string Sql1 = "create view vwpurchase as select top 1 HSNID as HsnCode,tblPurchaseItem.Qty,tblPurchaseItem.PRate,tblPurchaseItem.MRP,ItemDiscount as [Item Discount],NonTaxable as [Non Taxable],Taxable,GrossAmount as [Gross Amount],TaxPer,TaxAmount as [Tax Amount],NetAmount from tblPurchase join tblPurchaseItem on tblPurchase.InvId = tblPurchaseItem.InvID join tblItemMaster on tblItemMaster.ItemID = tblPurchaseItem.ItemId";
                SqlConnection conn = new SqlConnection(constr);
                conn.Open();
                //SqlCommand cmd = new SqlCommand(Sql, conn);
                SqlCommand cmd1 = new SqlCommand(Sql1, conn);
        ...
        private void rdbTax_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //string Sql = "DROP VIEW vwpurchase";
                string Sql1 = "create view vwpurchase as select top 1 Mop,VchType as [Voucher Type],Party,PartyCode as [Party Code],PartyAddress as [Party Address],PartyGSTIN as [Party GSTIN],BillType as [Bill Type],TaxAmt as [Tax Amount],GrossAmt as [Gross Amount],ItemDiscountTotal as [Item Discount Total],NonTaxable as [Non Taxable],Taxable,CGSTTaxPer as [CGST Tax Per],CGSTTaxAmt as [CGST Tax Amt],SGSTTaxPer as [SGST Tax Per],SGSTTaxAmt as [SGST Tax Amt],IGSTTaxPer as[IGST Tax Per],IGSTTaxAmt as [IGST Tax Amt],CashDiscount as [Cash Discount],OtherExpense as [Other Expense],NetAmount as [Net Amount],RoundOff as [Round Off] ,BillAmt as [Bill Amount] from tblPurchase join tblPurchaseItem on tblPurchase.InvId = tblPurchaseItem.InvID";
                SqlConnection conn = new SqlConnection(constr);
                conn.Open();
                //SqlCommand cmd = new SqlCommand(Sql, conn);
                SqlCommand cmd1 = new SqlCommand(Sql1, conn);
