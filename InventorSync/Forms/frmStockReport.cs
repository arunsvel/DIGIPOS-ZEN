        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rdoStockValue.Checked == true)
                {
                    ...
                    {
                        //string Sql1 = "create view vwstock as select CCName as[Cost Center],ItemCode as[Item Code],ItemName as [Item Name],tblStock.BatchUnique,cast(tblStock.MRP as numeric(36,2)) as MRP,cast (sum(QtyIn-QtyOut) as numeric(36,2))  as QOH,cast(tblStock.prate as numeric(36,2)) as [Purchase Rate],cast(((sum(QtyIn-QtyOut)*tblStock.PRate))as numeric(36,2)) as [Purchase Value],cast(tblStock.CostRateExcl as numeric(36,2)) as [Cost Rate Inclusive],cast(sum(QtyIn-QtyOut)*tblStock.CostRateExcl as numeric(36,2)) as [Cost Value],cast(tblStock.TaxPer as numeric(36, 2)) as [Tax Per],cast((sum(QtyIn-QtyOut)*tblStock.PRate)*tblStock.TaxPer/100 as numeric(36,2)) as [Tax Amount] from tblStockHistory join tblItemMaster on tblStockHistory.ItemID=tblItemMaster.ItemID join tblCostCentre on tblStockHistory.CCID=tblCostCentre.CCID join tblstock on tblStockHistory.BatchUnique=tblStock.BatchUnique where " + sqlitem + "  ID !=0 group by tblStockHistory.ItemID,ItemName,ItemCode,tblStock.PRate,tblStockHistory.CostRateExcl,tblStock.TaxPer,tblStock.MRP,tblStock.CCID,CCName,tblStock.BatchUnique ";
                        string Sql1 = @"create view vwstock as select CCName as[Cost Center],ItemCode as[Item Code],ItemName as [Item Name],tblStock.BatchUnique,cast(tblStock.MRP as numeric(36,2)) as MRP,cast (sum(QtyIn-QtyOut) as numeric(36,2))  as QOH, cast(tblStock.prate as numeric(36,2)) as [Purchase Rate], cast(((sum(QtyIn-QtyOut)*tblStock.PRate)) as numeric(36,2)) as [Purchase Value], 
                                            cast((sum(qtyin) - sum(qtyout)) * (sum(qtyin*tblStockHistory.CostRateExcl) / iif(sum(qtyin)=0,1,sum(qtyin)) )  as numeric(36,2)) as [Cost Rate Exclusive], 
                                            cast(sum(QtyIn-QtyOut)*(sum(qtyin) - sum(qtyout)) * (sum(qtyin*tblStockHistory.CostRateExcl) / iif(sum(qtyin)=0,1,sum(qtyin)) )  as numeric(36,2)) as [Cost Value],
                                            cast(tblStock.TaxPer as numeric(36, 2)) as [Tax Per],cast((sum(QtyIn-QtyOut)*tblStock.PRate)*tblStock.TaxPer/100 as numeric(36,2)) as [Tax Amount] 
                                        from tblStockHistory join tblItemMaster on tblStockHistory.ItemID=tblItemMaster.ItemID join tblCostCentre on tblStockHistory.CCID=tblCostCentre.CCID join tblstock on tblStockHistory.BatchUnique=tblStock.BatchUnique where " + sqlitem + "  ID !=0 group by tblStockHistory.ItemID,ItemName,ItemCode,tblStock.PRate,tblStock.CostRateExcl,tblStock.TaxPer,tblStock.MRP,tblStock.CCID,CCName,tblStock.BatchUnique ";
